    ...
    render() {
        return (
            <View style={kitchenStyles.container}>
                {this.state.scenarioPresent ? <Scenario 
                                               setScenerio={this.setScenerio} 
                                              /> : <View />}
                {this.state.upgradeMenu ? <Upgrade 
                                           setUpgrade={this.setUpgrade} 
                                          /> : <View></View>}
                {this.state.showStatMenu ? <StatScreen 
                ...
                <Text>Cronut semiotics raw denim, coloring book +1 williamsburg chartreuse before they sold out. Etsy cardigan gochujang master cleanse. Blog food truck PBR&B williamsburg helvetica. Tilde wayfarers freegan retro meggings, mlkshk hexagon polaroid street art vape single-origin coffee keytar unicorn. Biodiesel food truck tofu chicharrones gluten-free, ethical etsy locavore polaroid. Ramps poke tumblr, venmo hoodie kitsch chia four loko brooklyn copper mug. Sartorial cray hashtag migas venmo chicharrones godard cred thundercats biodiesel.</Text>
                {this.state.showStationOptions ? <StationOptions 
