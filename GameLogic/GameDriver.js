...
let GameDriver = {
    ...
    tic: () => {
        ...
        mStats.setPPower(mStats.getPPower() + Math.round(0.12 * mStats.getPEffectivness() * busyness.getBusynessReward(mStats)));
        if(pFrameLong >= 10){
            pFrameLong = 0;
            GameDriver.longTic();
        }
    ...
    endLevel: () => {
        ...
        mStats.getShiftCharacters().forEach((character) => {//This is why endDay index was subtracted by one. that sub 1 index is being handle right now
